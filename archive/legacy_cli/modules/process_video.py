...
class VideoFloodAnalyzer:
    ...
    def __init__(self, model_path="severity_model.pth", use_hybrid=True, use_production=True):
        if use_production and ProductionFloodAnalyzer is not None:
            self.analyzer = ProductionFloodAnalyzer(model_path=model_path)
        else:
    ...
    def process_video(self, video_path, output_csv="video_analysis.csv",
        ...
        camera_id = Path(video_path).stem or "video"
        ...
        try:
            while True:
                ...
                event_summary = None
                if analysis.get("water_detected") or float(analysis.get("depth_cm", 0) or 0) > 0:
                    frame_bytes = cv2.imencode(".jpg", frame)[1].tobytes() if frame is not None else b""
                    ...
                    event_response = self.aggregator.push(
                        payload=payload,
                        depth_cm=float(analysis.get("depth_cm", 0.0)),
                        confidence=float(analysis.get("water_confidence", 0.0)),
                        event_ts=frame_num / fps if fps > 0 else frame_num,
                    )
                    if event_response is not None:
                        event_summary = {
                            "camera_id": event_response.camera_id,
                            "frame_count": event_response.frame_count,
                            "estimated_flood_depth": event_response.estimated_flood_depth,
                            "confidence_score": event_response.confidence_score,
                            "next_action_recommendation": event_response.next_action_recommendation,
                            "burst_triggered": True,
                        }
                        self.event_summaries.append(event_summary)

                analysis["event_aggregated"] = event_summary is not None
                analysis["event_frame_count"] = event_summary["frame_count"] if event_summary else 0
                analysis["event_depth_m"] = event_summary["estimated_flood_depth"] if event_summary else None
                analysis["event_confidence"] = event_summary["confidence_score"] if event_summary else None
                analysis["event_action"] = event_summary["next_action_recommendation"] if event_summary else None
                ...
                results.append({
                    ...
                    'event_aggregated': analysis.get('event_aggregated', False),
                    'event_frame_count': analysis.get('event_frame_count', 0),
                    'event_depth_m': analysis.get('event_depth_m'),
                    'event_confidence': analysis.get('event_confidence'),
                    'event_action': analysis.get('event_action'),
                    'production_depth_cm': analysis.get('production_depth_cm'),
                    'production_confidence': analysis.get('production_confidence'),
                    'production_method': analysis.get('production_method'),
                    'production_action': analysis.get('production_action'),
                    'production_reference_estimate': analysis.get('production_reference_estimate'),
                })
        ...
        print(f"  Aggregated burst events: {len(self.event_summaries)}")
