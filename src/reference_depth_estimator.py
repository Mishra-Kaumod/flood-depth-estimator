...
class ReferenceDepthEstimator:
    ...
    def _detect_water(self, img: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        grey = cv2.inRange(hsv, np.array([0, 0, 30]), np.array([180, 45, 160]))
        grey[: int(h_img * 0.30)] = 0
        teal = cv2.inRange(hsv, np.array([75, 20, 40]), np.array([100, 150, 200]))

        combined = cv2.bitwise_or(blue, brown)
        combined = cv2.bitwise_or(combined, grey)
        ...
        mask[: int(h_img * 0.30)] = 0
        mask = self._validate_water_region(mask, img)
        water_frac = float(mask.mean())
        return mask, water_frac

    def _validate_water_region(self, mask: np.ndarray, img: np.ndarray) -> np.ndarray:
        """Prune likely false positives and keep the dominant lower-half water region."""
        if mask.size == 0:
            return mask
        ...
        h, _ = cleaned.shape
        cleaned[: int(h * 0.30)] = 0

        if np.count_nonzero(cleaned) < 50:
            return cleaned

        gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY).astype(np.float32)
        lap = np.abs(cv2.Laplacian(gray, cv2.CV_32F))
        smooth = cv2.GaussianBlur(lap, (9, 9), 0)
        texture_thresh = float(np.percentile(smooth[cleaned > 0], 70)) if np.any(cleaned > 0) else 18.0
        refined = np.where((cleaned > 0) & (smooth <= max(texture_thresh, 18.0)), 1, 0).astype(np.uint8)
        ...
        keep_mask = np.zeros_like(refined)
        keep_mask[labels == largest_component] = 1
        return keep_mask
    ...
    def _find_waterline(self, water_mask: np.ndarray, h: int) -> Tuple[int, float]:
        ...
        """
        row_density = water_mask.mean(axis=1)
        kernel = np.ones(9) / 9
        smooth = np.convolve(row_density, kernel, mode='same')

        lower_bound = int(h * 0.60)
        candidate_rows = np.where(smooth[:lower_bound] > 0.12)[0]

        if len(candidate_rows) == 0:
            candidate_rows = np.where(smooth > 0.08)[0]
            confidence = 0.45
        else:
            confidence = 0.65
        ...
        if waterline_y < h:
            ...
            if above_slice.size > 0 and below_slice.size > 0:
                ...
                if sharpness > 0.18:
                    confidence = min(confidence + 0.12, 0.80)
