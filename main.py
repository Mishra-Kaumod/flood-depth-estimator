...
def process_single_image(image_path, model_path="severity_model.pth", storage_mode="local", s3_handler=None):
    ...
    if not result['water_detected']:
        ...
        obj_info = result.get("depth_details", {}) \
            .get("method_2_object", {}) \
            .get("object_info")

        if obj_info:
            print("\nReference method:", obj_info.get("method"))
            print("Reference confidence:", obj_info.get("confidence"))
            print("Visible height (cm):", obj_info.get("visible_height_cm"))
